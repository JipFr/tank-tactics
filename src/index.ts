...
import { Client, Formatters, Guild, MessageEmbed, TextChannel } from 'discord.js';
    ...
    for await (const dir of commandFiles) {
        ...
        if (command.commandData !== undefined && typeof command.executeCommand == 'function') {
            ...
            console.log(
                red(
                    `${
                        command.commandData?.name ?? basename(dir.fullPath, '.js')
                    } is missing data to register the command!`,
                ),
            );
    }
})();
...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isCommand()) {
        ...
        if (interaction.customId === 'view-ap') {
            ...
            interaction.reply({
                embeds: [new MessageEmbed().setDescription(Formatters.codeBlock(tableString))],
                ephemeral: true,
            });
        }
    }
